...
if (carrosselWrapper && carrosselTrack && typeof equipeMembros !== "undefined" && equipeMembros.length) {
    ...
    function montarCarrossel() {
        ...
        equipeMembros.forEach((membro) => {
            ...
            const selos = Array.isArray(membro.selos) ? membro.selos : [];
            const selosHtml = selos.length
                ? `<div class="membro-card-selos" aria-label="Selos de ${membro.nome}">
                    ${selos.map((selo) => `<span class="membro-card-selo"><i class="fas fa-award"></i>${selo}</span>`).join("")}
                   </div>`
                : "";

            card.innerHTML = `
                <div class="membro-card-avatar">
                    <img src="${membro.foto}" alt="${membro.nome}" loading="lazy" onerror="this.onerror=null;this.src='./src/images/equipe/avatar/default-avatar.png';">
                </div>
                ...
                ${selosHtml}
                <div class="membro-card-redes ${redesHtml ? '' : 'sem-redes'}">${redesHtml}</div>
            `;
