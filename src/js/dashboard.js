...
function renderizarPerfilPublico(dados) {
    ...
    const projetosHtml = projetos.length
        ? projetos.map((projeto) => `
            <a class="projeto-item" href="projetos.html?projeto=${encodeURIComponent(projeto.slug || projeto.id)}">
                <img src="${escapeHTML(normalizarUrlImagem(projeto.logo_url, './src/images/LOGO_LSD.svg'))}"
                    alt="Logo do projeto ${escapeHTML(projeto.nome || 'Projeto')}">
                <span>
                    <strong>${escapeHTML(projeto.nome || "Projeto")}</strong>
                    <small>${escapeHTML({
                        em_desenvolvimento: "Em desenvolvimento",
                        em_producao: "Em produção",
                        concluido: "Concluído"
                    }[projeto.status] || "Projeto LSD")}</small>
                </span>
        ...
        : `
            <div class="perfil-secao-vazia">
                ...
                Este membro ainda não está vinculado a um projeto.
            </div>
        `;
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        const membroInicial = parametros.get("membro");
        let abaInicial = parametros.get("aba") || sessionStorage.getItem(ABA_STORAGE_KEY) || "kanban";

        if (membroInicial) abaInicial = "membros";
        ...
        if (abaInicial === "kanban") {
            ...
            await carregarDiretorioMembros();
            membrosCarregados = true;
            if (membroInicial) await abrirPerfilMembro(membroInicial);
