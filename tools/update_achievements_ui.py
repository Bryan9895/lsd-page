...
# ============================================================
# BACKEND — percentual de membros que desbloquearam a conquista
# ============================================================
app_path = ROOT / "backend" / "app.py"
app = app_path.read_text(encoding="utf-8")

if 'dados_conquista["percentual_desbloqueio"]' not in app:
    app = replace_once(
        app,
        '''    conquistas = verificar_conquistas(membro)\n\n    # Projetos terão modelo próprio futuramente.''',
...
# ============================================================
# FRONTEND — modal clicável inspirado nos achievements do GitHub
# ============================================================
js_path = ROOT / "src" / "js" / "dashboard.js"
js = js_path.read_text(encoding="utf-8")

if "function abrirDetalhesConquista(" not in js:
    marker = "\n\nfunction renderizarPerfilPublico(dados) {"
    if marker not in js:
        raise RuntimeError("renderizarPerfilPublico não encontrado")
...
if 'class="conquista-nome"' not in js:
    pattern = re.compile(
        r'    const conquistasHtml = conquistas\.length\n.*?\n    const projetosHtml = projetos\.length',
        re.S
    )
    replacement = r'''    const conquistasHtml = conquistas.length
    ...
    if count != 1:
        raise RuntimeError("Bloco de conquistas do perfil não encontrado")

if "abrirDetalhesConquista(conquista, membro);" not in js:
    ...
    js = replace_once(js, old_end, new_end, "fim de renderizarPerfilPublico")

js_path.write_text(js, encoding="utf-8")


# ============================================================
# CSS — nome abaixo do selo e visual do modal
# ============================================================
css_path = ROOT / "src" / "css" / "dashboard.css"
css = css_path.read_text(encoding="utf-8")

css = re.sub(
    r'(\.perfil-publico \.conquista-item \{.*?cursor:)\s*help;',
    r'\1 pointer;',
    css,
    count=1,
    flags=re.S
...
css = re.sub(
    r'(\.perfil-publico \.conquista-item strong \{.*?color:)\s*#F0F6FC;',
    r'\1 #24292F;',
    css,
    count=1,
    flags=re.S
...
if ".conquista-detalhe-overlay {" not in css:
    marker_css = "/* ---------- Tema escuro ---------- */"
    if marker_css not in css:
        raise RuntimeError("Marcador de tema escuro não encontrado")

    modal_css = r'''
/* ---------- Detalhes de conquistas no perfil público — v2.5.7 ---------- */
.perfil-publico .conquista-item {
    border-radius: 10px;
    cursor: pointer;
    transition: transform .18s ease, background .18s ease, box-shadow .18s ease;
}

.perfil-publico .conquista-item:hover,
.perfil-publico .conquista-item:focus-visible {
    background: rgba(88, 166, 255, .07);
    box-shadow: 0 0 0 1px rgba(88, 166, 255, .13);
    transform: translateY(-2px);
}

.perfil-publico .conquista-item:focus-visible {
    outline: 3px solid rgba(88, 166, 255, .26);
    outline-offset: 2px;
}

.perfil-publico .conquista-nome {
    display: -webkit-box;
    min-height: 2.5em;
    color: #24292F;
    font-size: .72rem;
...
body.conquista-modal-aberto { overflow: hidden; }

.conquista-detalhe-overlay {
    position: fixed;
    inset: 0;
    z-index: 180;
    display: grid;
...
.conquista-detalhe-overlay.visivel { opacity: 1; }

.conquista-detalhe-modal {
    position: relative;
    width: min(440px, 100%);
...
.conquista-detalhe-modal.visivel {
    opacity: 1;
    transform: translateY(0) scale(1);
}

.conquista-detalhe-fechar {
    position: absolute;
    top: 12px;
    right: 12px;
    z-index: 4;
...
.conquista-detalhe-fechar:hover,
.conquista-detalhe-fechar:focus-visible {
    background: rgba(13, 17, 23, .78);
    outline: 2px solid #fff;
    outline-offset: 1px;
}

.conquista-detalhe-hero {
    display: grid;
    place-items: center;
    min-height: 180px;
    padding: 28px;
...
.conquista-detalhe-modal.raridade-comum .conquista-detalhe-hero { background: linear-gradient(135deg, #8C959F, #57606A); }
.conquista-detalhe-modal.raridade-incomum .conquista-detalhe-hero { background: linear-gradient(135deg, #3FB950, #238636); }
.conquista-detalhe-modal.raridade-raro .conquista-detalhe-hero { background: linear-gradient(135deg, #58A6FF, #1F6FEB); }
.conquista-detalhe-modal.raridade-ouro .conquista-detalhe-hero,
.conquista-detalhe-modal.raridade-diamante .conquista-detalhe-hero { background: linear-gradient(135deg, #F2CC60, #D29922); }
.conquista-detalhe-modal.raridade-epico .conquista-detalhe-hero { background: linear-gradient(135deg, #A371F7, #8957E5); }
.conquista-detalhe-modal.raridade-lendario .conquista-detalhe-hero { background: linear-gradient(135deg, #F0883E, #A371F7); }
.conquista-detalhe-modal.raridade-especial .conquista-detalhe-hero { background: linear-gradient(135deg, #FF7B72, #A371F7); }
.conquista-detalhe-modal.raridade-secreto .conquista-detalhe-hero { background: linear-gradient(135deg, #21262D, #6E40C9); }

.conquista-detalhe-selo {
    display: grid;
    place-items: center;
    width: 116px;
    height: 116px;
...
.conquista-detalhe-selo > i { font-size: 46px; color: #F2CC60; }
.conquista-detalhe-corpo { padding: 22px 18px 20px; }

.conquista-detalhe-titulo-linha {
    display: flex;
...
.conquista-detalhe-titulo-linha h3 {
    margin: 0;
    color: #F0F6FC;
    font-size: 1.23rem;
    line-height: 1.25;
...
.conquista-detalhe-divisor {
    height: 1px;
    margin: 17px 0;
    background: #30363D;
}

.conquista-detalhe-historico h4 {
    margin: 0 0 14px;
    color: #8B949E;
    font-size: .78rem;
    font-weight: 800;
...
.conquista-detalhe-status,
.conquista-detalhe-evento {
    display: flex;
    align-items: flex-start;
    gap: 10px;
}

.conquista-detalhe-status { color: #8B949E; font-size: .77rem; }
.conquista-detalhe-status > i { width: 18px; margin-top: 2px; color: #8B949E; text-align: center; }
.conquista-detalhe-status strong { color: #3FB950; }
.conquista-detalhe-status span { color: #8B949E; }
...
.conquista-detalhe-evento::before {
    position: absolute;
    top: -13px;
    bottom: 9px;
    left: 3px;
...
.conquista-detalhe-evento p { margin: 0; color: #8B949E; font-size: .77rem; line-height: 1.45; }
.conquista-detalhe-evento strong { color: #58A6FF; font-weight: 600; }

html.tema-escuro .perfil-publico .conquista-nome,
html.tema-escuro .perfil-publico .conquista-item strong { color: #F0F6FC; }

html.tema-escuro .perfil-publico .conquista-item:hover,
html.tema-escuro .perfil-publico .conquista-item:focus-visible { background: rgba(88, 166, 255, .08); }

@media (max-width: 520px) {
    .conquista-detalhe-overlay { align-items: end; padding: 0; }
    .conquista-detalhe-modal {
        width: 100%;
        max-height: 88dvh;
        border-right: 0;
    ...
    .conquista-detalhe-hero { min-height: 156px; }
}

@media (prefers-reduced-motion: reduce) {
    .conquista-detalhe-overlay,
    .conquista-detalhe-modal,
    .perfil-publico .conquista-item { transition: none !important; }
}

'''
    css = css.replace(marker_css, modal_css + marker_css, 1)

css_path.write_text(css, encoding="utf-8")


# ============================================================
# README — versão atual e resumo da funcionalidade
# ============================================================
...
readme = re.sub(
    r'A versão \*\*2\.5\.4\*\* reúne melhorias de usabilidade no dashboard e recuperação de senha por e-mail, mantendo os recursos de colaboração, administração e backups\.',
...
readme = readme.replace("# Versão atual — 2.5.4", "# Versão atual — 2.5.7", 1)

if "## Novidades da 2.5.7" not in readme:
    marker_readme = "# Versão atual — 2.5.7\n\n"
    novidades = '''## Novidades da 2.5.7
...
'''
    readme = replace_once(readme, marker_readme, marker_readme + novidades, "versão atual do README")

readme_path.write_text(readme, encoding="utf-8")


# Validação antes do commit automático da branch temporária.
subprocess.run(["python", "-m", "py_compile", "backend/app.py"], cwd=ROOT, check=True)
subprocess.run(["node", "--check", "src/js/dashboard.js"], cwd=ROOT, check=True)

if ".conquista-detalhe-overlay" not in css:
    raise RuntimeError("CSS do modal não foi aplicado")
if "Versão atual — 2.5.7" not in readme:
    raise RuntimeError("README não foi atualizado para 2.5.7")

# O workflow base adiciona JS/CSS/imagens. Estes dois arquivos também precisam
# entrar no commit temporário para que a árvore final contenha toda a feature.
